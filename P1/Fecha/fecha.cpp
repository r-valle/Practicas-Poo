...
// si se crea un nuevo objeto, no tiene sentido devolver una referencia a él, sino devolver el mismo objeto
...
// operador decremento fecha menos nº de días, el cual llama al operador decremento sufijo ya declarado
...
istream& operator>>(istream& flujo, Fecha& f1){
  // tamaño de la cadena según el formato indicado en el enunciado
  // a saber: DD/MM/AAAA (= 10 caracteres) + el carácter terminador
  unsigned tamano = 11;
  ...
  flujo.width(tamano);
  // Introducimos flujo en la cadena
  flujo >> cadena;
  f1 = cadena;
  return flujo;
}
...
ostream& operator<<(ostream& flujo, const Fecha& f1){
  // Insertamos en flujo la cadena correspondiente a la fecha f1
  flujo << f1.cadena();
  return flujo;
}