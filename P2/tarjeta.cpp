...
// función que representa al algoritmo de Luhn
// necesario para validar el número de tarjeta
...
Numero::Numero(const Cadena& num) throw(Incorrecto){
  ...
  // Primero comprobamos los blancos que pueda tener cad
  for(unsigned i = 0; i < num.length(); ++i){
    if(isdigit(num[i])){		// es un número
      aux[j++] = num[i];
