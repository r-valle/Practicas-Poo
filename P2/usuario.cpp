...
// Constructor
Usuario::Usuario(Cadena id, Cadena nombre, Cadena apellidos, Cadena direccion, Clave contrasena) throw(Id_duplicado)
...
/*  pair<set<Cadena>::iterator,bool> user = ids_.insert(id)*/
  
  if(ids_.insert(id).second == false){
...
ostream& mostrar_carro(ostream& flujo, const Usuario& user){
  ...
  for(map<Articulo*, unsigned>::const_iterator i = user.compra().begin(); i != user.compra().end(); ++i){
    flujo << setfill(' ') << setw(4) << i->second << "   " << *(i->first) << endl;
 }
  return flujo;
}